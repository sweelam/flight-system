...
public final class EmailBuilders {
    ...
    public static String buildEmailEvent(final String customerEmail, final String flightName) {
        ...
        return gson.toJson(
                new EmailEvent(customerEmail, flightName,
                        MESSAGE_BODY.formatted(flightName, now),
                        APPLICATION_NAME, now.toString()));
    }

    private record EmailEvent(String customerEmail,
            String flightUuid,
            String messageBody,
            String producer,
            String eventTime) {
